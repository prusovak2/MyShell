...
int main(int argc, char **argv) {
  ...
  int option;
  //  ':' at the starting of the string so compiler can distinguish between '?'
  //  and ':'
  while ((option = getopt(argc, argv, ":c:")) != -1) {
    ...
    case ':':
      errx(2, "-%c: option requires an argument",
           optopt); // 2 is ret val of bash for unknown option or missing oprarg
...
void handle_sigint(int sig) {
  ...
  rl_redisplay();
  lastRetVal =
      128 + sig; // should this be here? ctrl+c changes ret val of last cmd...
}